...
def main():
    ...
    # Updated 12-column fallback map
    expected_cols = [
        "time_ms",
        "loop_us",
        "target1",
    ...
    if "time_ms" not in df.columns or len(df.columns) < 12:
        print(
            "Warning: Standard CSV headers not found or incomplete. Re-mapping 12 columns by position..."
        )
    ...
    ax1.plot(
        df["time_s"],
        df["angle1"],
        color=color_actual_1,
        linewidth=2.0,
        label="Actual Angle 1",
    )
    ax1.set_ylabel("Angle (Degrees)", color="black", fontsize=11, fontweight="bold")
    ax1.tick_params(axis="y", labelcolor="black")
    ax1.grid(True, linestyle=":", alpha=0.6)
    ax1.set_title("Motor 1 (Yaw) Dynamics", fontsize=13, fontweight="bold", pad=10)
    ...
    ax3.plot(
        df["time_s"],
        df["angle2"],
        color=color_actual_2,
        linewidth=2.0,
        label="Actual Angle 2",
    )
    ...
    ax3.set_title("Motor 2 (Roll) Dynamics", fontsize=13, fontweight="bold", pad=10)
    ...
    output_image = csv_file.replace(".csv", "_basic.png")
    plt.savefig(output_image, dpi=300, bbox_inches="tight")
    print(f"Basic plot saved successfully as: {output_image}")
    plt.show()
